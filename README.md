...
本プラットフォームの根源的な目的は、**誰もが自由に使える著作権フリーの学習問題を蓄積・共有する**ことです。そのために、生成された問題は以下のプロセスを経て公開されます。

-   **創作性の高い問題生成**: AIは原資料の「アイデア」や「事実」に基づいて問題を生成します。文章の表現をそのまま複製・転載するのではなく、内容を理解した上で**新たな表現で問題を創作**します。著作権はアイデアや事実そのものではなく、その「表現」を保護するものであるため、このプロセスは適法であると考えています。

-   **運営による著作権レビュー**: 生成された全ての問題は、プラットフォーム上で公開される前に、**運営チームによるレビュー**が行われます。このチェックにより、元の著作物の表現と酷似していないか、意図せず著作権を侵害する可能性がないかを確認し、安全性を担保します。

-   **著作権フリーでの公開**: レビューを通過した高品質な問題セットは、**著作権フリーのコンテンツとしてプラットフォーム上で一般公開**されます。これにより、世界中の学生や教育者が、いつでも誰でも無料でアクセスし、学習に活用できるオープンなナレッジベースが構築されます。

### 🔒 安全設計のまとめ

-   **入力データ（原資料）**: **【非公開】**。厳格なアクセス制御下で保管。目的は問題生成と著作権侵害対応のみ。
-   **生成された問題**: **【著作権フリーで公開】**。AIによる「創作」、運営による「レビュー」を経て、安全な学習リソースとして共有。

このような透明性の高いプロセスを通じて、EduMintは教育リソースのオープン化を推進し、知の共有に貢献します。
