...
EduMintは、著作権法に最大限配慮した設計となっています。

- **アップロードされた資料（講義ノート・試験問題などの原本）は、サービス運営者によって一切閲覧・保持されません**。
- 原本は、ユーザー端末から直接AIベンダー（Google Gemini API）へ送信され、**運営者サーバーは中継も保存もしません**。
- これにより、AIサービス運営者（EduMint）は、アップロード資料を「公衆に送信」しておらず、**著作権法第23条（公衆送信権）に抵触するリスクは極めて低くなっています**。

### ✅ 公衆送信に該当しない理由

- **クラウドストレージへの私的アップロードと同等の扱い**となるよう、第三者がアクセスできない構成を維持しています。
- AIによって抽出・分析されるのはメタデータであり、**生成される問題文は原著作物の「翻案」ではなく、内容的に新たに構成された著作物**です。
- ユーザーが入力した資料は、**AIモデルのトレーニングには一切使用されません（Gemini APIの契約条件に準拠）**。

### 🔒 安全設計のポイント

- **ゼロアクセスアーキテクチャ**: AIベンダーとの通信はバックエンドワーカーから直接行われ、API Gatewayやフロントエンドからは入力データへアクセスできない設計です。
- **一時的なメモリ処理のみ**: 入力データは保存されず、AI処理後に即時削除されます。
- **抽出された情報（構成案・問題の要素）は全て再構成され、元の文章を引用・転載することはありません**。

このような設計により、EduMintは教育支援ツールとして**著作権の保護と実用性のバランスを両立**しています。
